// base_mega1.ino
//
// Arduino Mega #1 - rover base controller.
//   * 6x 12V/83RPM DFRobot gear motors, all driven; only the two fixed
//     middle wheels (ML, MR) carry quadrature encoders - see the
//     encoder section below for why
//   * 3x DFRobot DRI0002 Dual H-Bridge V1.4 motor drivers, L298N-based
//     (6 motor channels total)
//   * 4x 40kg steering servos (front-left/right, rear-left/right),
//     driven via a PCA9685 16-channel PWM driver module over I2C
//     (address 0x40, its factory default - see the wiring note below
//     for why this project deliberately does NOT jumper it to
//     anything else) rather than direct Mega pins. Channels 0-3 -> FL/FR/RL/RR.
//     ServoEasing doesn't work with a PCA9685 (it's built for direct-
//     pin, standard-Servo-library-style control, not an I2C PWM chip),
//     so smoothed movement is reimplemented here as a small,
//     non-blocking millis()-based ramp (updateSteerEasing()) toward
//     each corner's latest commanded position, rather than dropped -
//     this project has consistently valued smoothed servo motion
//     (mast lift, microscope cover, and this exact axis previously),
//     and there's no reason a different output mechanism should mean
//     losing it. A genuine side effect worth noting: since nothing
//     else on this board used ServoEasing, this removes that GPL-3.0
//     dependency here entirely - see the journal for the history if
//     that matters (docs/journal.md).
//   * DS18B20 temperature sensor (board/enclosure temperature, not
//     ambient air - it's whatever's near wherever it's actually
//     mounted), TO-92 package, 1-Wire on a single digital data pin
//     (kDs18b20DataPin) rather than I2C. Previously pin 20 (this
//     Mega's own SDA), moved to A4 when the PCA9685 above needed 20
//     back for genuine I2C - A4 was one of the pins the steering
//     servos vacated moving to that same PCA9685, so this reuses one
//     freed pin for another, rather than search for a third.
//     REQUIRES AN EXTERNAL 4.7k-OHM PULL-UP RESISTOR between DQ and
//     VDD - the Arduino's internal pull-ups are too weak for reliable
//     1-Wire timing; this is not optional or achievable via
//     INPUT_PULLUP. Conversion is read via a non-blocking two-phase
//     state machine (request, then check back after the datasheet's
//     worst-case ~750ms conversion time) rather than the library's own
//     default blocking `delay(750)` pattern, since blocking loop() for
//     750ms on every read would stall drive-command processing on a
//     board whose real job is real-time motor control. See the
//     temperature section further down for the full reasoning - it
//     applies identically on all five boards with this same sensor.
//   * Cooling fan via a generic N-channel MOSFET driver module (e.g.
//     IRF520-style: SIG/VCC/GND control side, V+/V- screw terminals
//     for the load) - same automatic, thermostatic design and the
//     same low-side-switch wiring caveat as mast_uno3.ino's own copy
//     of this feature (see that file's header for the full reasoning
//     on the wiring, hysteresis, and fail-toward-running-on-sensor-
//     failure choice; identical here). The one real difference: this
//     board is a Mega with pins to spare, so kFanPwmPin (44) gets
//     genuine hardware PWM via analogWrite() - no need for the
//     mast's software-PWM workaround, which existed only because that
//     Uno's entire hardware-PWM budget was already spent.
//
// DRI0002 interface (per DFRobot's own datasheet/wiki): each of its two
// channels exposes just ONE direction pin (Mx) plus one PWM/speed pin
// (Ex) - not a pair of direction pins as some other dual-H-bridge
// boards use. Documented convention: Mx = LOW drives that channel
// "forward". That's only meaningful relative to which wire landed on
// the +/- screw terminal, so confirm actual wheel rotation direction
// on the bench and flip the sign in setWheelThrottle() below (not by
// rewiring the terminal) if a wheel spins the wrong way.
//
// Wiring note: DRI0002 has a logic-supply jumper - shorted means the
// L298N's logic rail is powered from the same supply as the motors,
// which the datasheet says is only safe below 12V. Our motors are
// rated exactly at 12V, right at that boundary, so the safer default
// is to leave the jumper OPEN and power the driver's logic pins from
// the Arduino's own 5V rail instead (shared ground required either
// way). Confirm against the specific V1.4 silkscreen before wiring.
//
// Wiring note for the servos: the PCA9685 module's V+ screw terminal
// is a dedicated servo power rail, separate from its VCC (logic-only)
// pin - wire V+ to a separate, adequately-rated 5-6V supply (four
// 40kg-class digital servos can draw well beyond what the Mega's
// onboard 5V regulator or a USB-supplied 5V can safely deliver),
// sharing ground with the Arduino, never from the Mega's own 5V pin.
// This is exactly the dedicated rail a previous, direct-pin version of
// this same steering setup didn't have, and had to call out as a gap
// to fill separately - the PCA9685 fills it natively.
//
// I2C wiring: SDA -> Mega pin 20, SCL -> Mega pin 21 (the Mega's
// dedicated hardware I2C pins, not configurable - this is why the
// DS18B20 above lives on A4 now instead, having previously used pin
// 20 back when nothing else needed it). The PCA9685's I2C address is
// jumper-configurable via solder pads on the board itself, but this
// project deliberately leaves them unbridged: 0x40, the factory
// default, is used as-is - see kPca9685Address below for why (in
// short, this chip's own separate "All Call" address is a fixed 0x70
// on every unit regardless of jumpers, so 0x70 is specifically the
// one address NOT to jumper a device to, per the chip's own datasheet
// and Adafruit's own guidance - an earlier version of this comment
// got that backwards).
//
// Talks to the ROS 2 `rover_base` bridge node over USB-serial using the
// shared RoverProtocol framing (see firmware/common/RoverProtocol).
//
// Requires "Adafruit PWM Servo Driver Library" (Library Manager, by
// Adafruit) for the PCA9685, "OneWire" (by Paul Stoffregen) and
// "DallasTemperature" (by Miles Burton) for the temperature sensor -
// see the LICENSING NOTE below for how these differ from each other
// and from the rest of this project.
//
// LICENSING NOTE: this file no longer uses ServoEasing (GPL-3.0) -
// moving the steering servos to the PCA9685 removed that dependency
// entirely, since nothing else on this board used it. What remains:
// "Adafruit PWM Servo Driver Library" is BSD licensed (confirmed
// against the library's own source headers, not just a third-party
// index - some of those show "NOASSERTION" for this library, which is
// a known quirk of automated license detection against a non-standard
// header format, not an actual absence of a license). BSD is
// permissive, similar in spirit to this project's own Apache-2.0,
// though a technically distinct license family - worth a line here
// for completeness even though it's a materially smaller concern than
// GPL/LGPL copyleft. DallasTemperature is LGPL-2.1, not MIT, not BSD,
// and not GPL-3.0 - see base_mega1.ino's earlier version of this same
// note (now in docs/journal.md's history) for the fuller reasoning on
// why that's worth tracking as its own, distinct consideration.
